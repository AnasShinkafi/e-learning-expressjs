...
exports.createLayout = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, nex) => {
    try {
        const { type } = req.body;
        const isTypeExist = await layout_model_1.default.findOne({ type });
        if (isTypeExist) {
            return nex(new ErrorHandler_1.default(`${type} already exist`, 400));
        }
        if (type === "Banner") {
        ...
        return nex(new ErrorHandler_1.default(error.message, 500));
    }
});
// Edit layout
exports.editLayout = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, nex) => {
    try {
        const { type } = req.body;
        if (type === "Banner") {
        ...
        if (type === "Categories") {
            ...
            await layout_model_1.default.findByIdAndUpdate(categoriesData?._id, { type: "Category", categories: categoriesItems });
        }
        res.status(200).json({
            success: true,
...
// Get layout by type
exports.getLayoutByType = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, nex) => {
    try {
